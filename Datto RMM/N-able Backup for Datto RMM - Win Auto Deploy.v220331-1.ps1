...
<# ----- Behavior: ----
    # D\L and deploy a NEW Backup Manager with system managed Passphrase encryption
    # Checks for existing installation and exits if found
...
if ((Test-Path "C:\Program Files\Backup Manager\config.ini" -PathType leaf) -eq $true) { Write-output "Backup Manager already present"; Break }
    ...
    Start-Process -FilePath $Path -ArgumentList "-unattended-mode -silent -partner-uid $env:Backup_UID $BackupProfile $BackupProduct" -passthru
    ## $env: prefix required to pass external variable from DATTO RMM to script
